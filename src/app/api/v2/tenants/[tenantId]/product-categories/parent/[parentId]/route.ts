...
export async function GET(
  req: NextRequest,
  { params }: { params: { tenantId: string; parentId: string } }
) {
  const { tenantId, parentId } = await params;
  const categoryController = getCategoryController();
  return await categoryController.getCategoriesByParent(req, tenantId, parentId);
}